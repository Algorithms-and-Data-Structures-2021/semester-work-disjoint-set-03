...
int main(int argc, char **argv) {
  ...
  if (argc != 4) {
    ...
    for (int i = 1; i <= num_datasets; i++) {
      // Открываем файлы
      if (i < 10) {
        output_file = ofstream(path_project + "/benchmark/test_results/join/" + str_num_elem + "/0" + to_string(i) + ".csv");
        join_dataset = ifstream(path + "/join/0" + to_string(i) + "/" + str_num_elem + ".csv");
      } else {
        output_file = ofstream(path_project + "/benchmark/test_results/join/" + str_num_elem + "/" + to_string(i) + ".csv");
      ...
      // Do <num_trials> fewer <join> operations,
      // since for N elements, you can make a maximum of n-1 joins,
      // and the following unions will not make sense
      int n = num_elem - num_trials;
      for (int j = 0; j < n; j++) {
        std::getline(join_dataset, join_line);
        int num1 = 0;
        int num2 = 0;
      ...
      auto dist = uniform_int_distribution(0, num_elem);

      // Measure the time of the operation of combining 2 random elements
